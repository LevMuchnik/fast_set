...
%	If a & b are both sorted, finds whether each element in a is also element of b and (optionally) it's location in b
%	Very efficient (usually, 5-14 times faster - performance difference grows with size) replacement of the standard MatLab ismember function. 
%
%	Receives:
%		a	- numeric vector of any type. Matrices ('rows' switch in union are not supported).
%		b	- numeric vector of any type (must be same time as a). Matrices ('rows' switch in union are not supported).
%	Notice, a and b must be of the same numeric type (both doubles, int16 , ...) and must be sorted (not verified). 
%
%	Returns:
